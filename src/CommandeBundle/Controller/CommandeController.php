...
class CommandeController extends Controller
{
    ...
    public function showMAction($idCmd)
    {
        $em  = $this->getDoctrine()->getManager();


        $cmd=$em->getRepository('CommandeBundle:Produit')->listProductById($idCmd);
    ...
    public function listcmdMAction()
    {
        $em = $this->getDoctrine()->getManager();
        $idU=0;
   ...
   /* public function userloginAction($password,$email)
    {
        $em = $this->getDoctrine()->getManager();

        $query = $em->createQuery("
            SELECT c FROM CommandeBundle:User c");
...
echo "true";
    ...
    public function userloginAction($email,$password)
    {
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery("
            SELECT c.email,c.id FROM CommandeBundle:User c where c.username=$email and c.password=$password");
        $user = $query->getResult();
    ...
    public function panierMAction()
    {
        ...
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($lcmd);
        return new JsonResponse($formatted);
    ...
    public function couponMAction($coupon)
    {
        $em  = $this->getDoctrine()->getManager();
    ...
    public function ajouterMAction( $idProduit,$id)
    {  $em = $this->getDoctrine()->getManager();
        $lcmd= new LigneCmd();
        $user = $this->get('security.token_storage')->getToken()->getUser();
        $prix = $em->getRepository('CommandeBundle:Produit')->getprix($idProduit);
        if ($user=="anon.")
        { $exist = $em->getRepository('CommandeBundle:LigneCmd')->checkcmd($idProduit, $id);
        ...
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($idProduit);
        return new JsonResponse($formatted);
    ...
    public function addqteMAction($idProduit)
    {
        $em = $this->getDoctrine()->getManager();
        $quantite=$em->getRepository('CommandeBundle:Produit')->getQteprod($idProduit);
        $quantitel=$em->getRepository('CommandeBundle:Produit')->getQte($idProduit);
    ...
    public function delqteMAction($idProduit)
    {
        $em = $this->getDoctrine()->getManager();
        $qte=$em->getRepository('CommandeBundle:LigneCmd')->getQte($idProduit);
        if ($qte>1){
    ...
    public function deletePMAction($idLigneCmd )
    {
            $em = $this->getDoctrine()->getManager();
        $commande = $em->getRepository('ProduitBundle:LigneCmd')->find($idLigneCmd);
            $em->remove($commande);
            $em->flush();
        $serializer = new Serializer([new ObjectNormalizer()]);
    ...
    public function idmobAction($username )
    {
        $em = $this->getDoctrine()->getManager();
        $id = $em->getRepository('CommandeBundle:User')->gitid($username);
    ...
    public function allAction()
    {
        $tasks = $this->getDoctrine()->getManager()
            ->getRepository('CommandeBundle:User')
            ->findAll();
        $serializer = new Serializer([new ObjectNormalizer()]);
