...
class CommandeRepository extends EntityRepository
{
    ...
    public function gitid($username)
    {
        $query = $this->getEntityManager()->createQuery("select u.id from CommandeBundle:User u where u.username =$username");
        return $query->getResult();
    }

    public function listProductById($idcmd)
    {
        $query = $this->getEntityManager()->createQuery("select p.nomProduit, p.prix ,p.description, p.image, c.qte,p.idProduit,c.etatCmd,c.dateCmd,c.montant from CommandeBundle:Commande c, CommandeBundle:Produit p where p.idProduit = c.idProduit AND c.idCmd=$idcmd");
        return $query->getResult();
    }
