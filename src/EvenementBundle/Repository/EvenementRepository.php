...
//STAT USER CONNECTE EVE
    ...
    public function rechercherlike($ideve,$iduser)
    {
        $query = $this->getEntityManager()->createQuery("select count(e.idLike) from EvenementBundle:LikeEve e where e.idEvenement='$ideve' and e.idUser='$iduser'  ");
        return $query->getSingleScalarResult();
    }

    public function rechercherliketype($ideve,$iduser)
    {
        $query = $this->getEntityManager()->createQuery("select e.type from EvenementBundle:LikeEve e where e.idEvenement='$ideve' and e.idUser='$iduser'  ");
        return $query->getSingleScalarResult();
    }


    public function rechercherlikeid($iduser,$ideve)
    {
        $query = $this->getEntityManager()->createQuery("select e.idLike from EvenementBundle:LikeEve e where e.idEvenement='$ideve' and e.idUser='$iduser'  ");
        return $query->getSingleScalarResult();
    }
    public function nbrlike($ideve)
    {
        $query = $this->getEntityManager()->createQuery("select count(e.idLike) from EvenementBundle:LikeEve e where e.idEvenement='$ideve' and e.type='1'  ");
        return $query->getSingleScalarResult();
    }
    public function nbrdislike($ideve)
    {
        $query = $this->getEntityManager()->createQuery("select count(e.idLike) from EvenementBundle:LikeEve e where e.idEvenement='$ideve' and e.type='2'  ");
        return $query->getSingleScalarResult();
    }
    ...
    {
        $query = $this->getEntityManager()->createQuery("select e.dateDebut,e.dateFin,e.idEvenement,e.nomEvenement,e.type,e.lieu,e.description,e.max,e.path,e.pseudoAdmin from EvenementBundle:Evenement e");
