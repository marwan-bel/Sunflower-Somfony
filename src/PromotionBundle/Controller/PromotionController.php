...
//AFFICHER tout les pro (front)
    ...
    public function afficherpromobileAction()
    {
    ...
    public function imgmobileAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $promotion=$this->getDoctrine()->getRepository(Promotion::class)->find($request->get('idpro'));
        ...
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($img);
        return new JsonResponse($formatted);
