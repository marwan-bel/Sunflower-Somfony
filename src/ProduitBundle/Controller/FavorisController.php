...
class FavorisController extends Controller
{
     ...
     *
     */
    public function indexmobAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $favoris = $em->getRepository('ProduitBundle:Favoris')->listFavorisofthisuser(36);
     ...
     *
     */
    public function existmobAction(Request $request){
        $em = $this->getDoctrine()->getManager();
        $idProduit= $request->get('idProduit');
        $exist=$em->getRepository('ProduitBundle:Favoris')->checkfav($idProduit,36);
        $serializer = new Serializer([new ObjectNormalizer()]);
    ...
    public function newmobAction(Request $request)
    {
        ...
        $favori->setIdProduit($idProduit);
        $favori->setNomProduit($nomP);
        $favori->setIdClient(36);

        $em->persist($favori);
        $em->flush();
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($favori);
        return new JsonResponse($formatted);
     ...
     *
     */
    public function deletemobAction(Favoris $favori)
    {
        $em = $this->getDoctrine()->getManager();
