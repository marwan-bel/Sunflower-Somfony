...
class ProduitRepository extends EntityRepository
{
    ...
    public function listProduitByCategorie($nomCategorie)
    {
        $query = $this->getEntityManager()->createQuery
        ("select p from ProduitBundle:Produit p where p.nomCategorie=$nomCategorie");
        return $query->getResult();

    }

    public function listmostratedProduit()
    {
        $query = $this->getEntityManager()->createQuery
        ("select p from ProduitBundle:Produit p ORDER BY p.rating DESC")->setMaxResults(5);
        return $query->getResult();

    }

    public function listnewestProduit()
    {
        $query = $this->getEntityManager()->createQuery
        ("select p from ProduitBundle:Produit p ORDER BY p.dateAjout DESC")->setMaxResults(5);
        return $query->getResult();
